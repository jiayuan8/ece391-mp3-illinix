...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        fs_start_addr = mod->mod_start;  // fs_start_addr is the starting address of file system
    ...
    filesystem_init((uint32_t)fs_start_addr);

    // enable cursor
    enable_cursor(0,CURSOR_MAX);
    /* Enable interrupts */
    /* Do not enable the following until after you have set up your
     * IDT correctly otherwise QEMU will triple fault and simple close
     * without showing you any output */

    // initialize scheduling, set frequency as 20Hz
    init_pit(20);
    //sti();
    // Reset_DSP();
    // play_music("testaudio");
    // Initialize terminal
    enable_irq(5);
    play_music("testaudio");
    init_terminal();
